...
namespace MunicipalArchive.Windows
{
    ...
    {
        private List<tblUser> _userData;
        private PersianDateTime _getDate;
        public bool OkLogin;
        public WinLogin()
        {
            InitializeComponent();
            _userData = new List<tblUser>();
            OkLogin = false;
        }
        ...
        private async void Window_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            Hide();
            var winAddAdmin = new WinAddAdmin {PostTypeId = 1};
                ...
                else
                {
                    ...
                    try
                    {
                        if (userLogin.Login())
                        {
                            ...
                            var user = _userData.FirstOrDefault(x => x.UserName == TxtUserName.Text);
                            if (user != null && user.User_PostType_Id == null)
                            {
                                mainWindow.BtnFile.Visibility = Visibility.Collapsed;
                                mainWindow.BtnPerson.Visibility = Visibility.Collapsed;
                                mainWindow.BtnAddUser.Visibility = Visibility.Collapsed;
                                mainWindow.BtnRestore.Visibility = Visibility.Collapsed;
                            }
                            OkLogin = true;
                            mainWindow.Show();
                            Close();
